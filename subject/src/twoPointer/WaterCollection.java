...
public class WaterCollection {
    ...
    /**
     * 双指针解法。在思路2的基础上，仔细思考，其实我们没必要维护所有位置的最大值。因为如果右边的柱子高度>左边的柱子高度，
     * 则蓄水高度必然是由左边的柱子高度所决定的。所以我们可以认为如果一端有更高的条形块（例如右端），
     * 积水的高度依赖于当前方向的高度（从左到右）。当我们发现另一侧（右侧）的条形块高度不是最高的，我们则开始从相反的方
     * 向遍历（从右到左）。所以我们只需要维护两个指针和两个变量，left指针和一个始终表示当前left位置左边[0,left]的最高点
     * 即left_max；right指针和一个始终表示当前right位置[right,n]右边的最高点。
     * 如果height[left]<height[right]，表示右侧始终高于左侧，其蓄水将由left_max决定，即h = left_max-height[left]，然后
     * left继续向右移动。反之，表示左侧高于右侧，h = right_max-height[right]，然后right继续向左移动。直至left>=right。
     * @param height
     * @return
     */
    public static int trap_doublePointer(int[] height){
