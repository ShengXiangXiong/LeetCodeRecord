...
14.可以使用toArray方法返回数组类型的对象，有不带参数和带参数的两种形式：
object[] toArray(){}   和  T[] toArray(T[]){}
我们可以看出来前者是直接通过返回一个object对象数组，其内部是构造的一个Object数组，然后进行数据拷贝，
此时若在外部进行向下转型就会产生ClassCastException。比如 return (int[][])toArray()
——下转注意事项：
- 向下转型的前提是父类对象指向的是子类对象（也就是说，在向下转型之前，它得先向上转型）——对上面而言也就是说object[]得指向
int[]，但是toArray是通过copy数据完成的，并没有指向int[]实例。
- 向下转型只能转型为本类对象（猫是不能变成狗的）。

而对于后者，我们可看出是通过泛型来实现的，因为相当于指定了向下转型的类型，此时return toArray(int[][])再向下转型就不会出错了
